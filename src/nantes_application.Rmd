...
# TODO get values from unique values of synth_pop
...
combi_diff <- combi_diff %>%
  arrange(c(factor(family_comp, levels = dec_fam_comp)), (factor(size, levels =
                                                                   dec_size)), (factor(age, levels = dec_age)), (factor(ownership, levels =
                                                                                                                          dec_owner)))
nrow(combi_diff)
head(combi_diff)
...
all_combinations <-
  expand.grid(dec_owner, dec_age, dec_size, dec_fam_comp)
...
constraint_composition <-
  eta_fonction(u = ech_compo, v = prob_comp1, w = p_compo)

constraint_statut <-
  eta_fonction(u = ech_statut, v = prob_owner1, w = p_statut)
